...
class Sprite(GameObject, pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, **kwargs):
        ...
        self.ignore_after_n_ticks = (
            1  # >0: not to be ignored; <=0: ignore this sprite for one tick
        )
        ...
        if "sprite_sheet" in kwargs:
            ...
            assert isinstance(
                sheet, SpriteSheet
            ), "ERROR: in Sprite's ctor: kwargs[`sprite_sheet`] must be of type `SpriteSheet`!"
            ...
            width_height = kwargs.get(
                "width_height", (self.spritesheet.tw, self.spritesheet.th)
            )
            self.rect = pygame.Rect(
                x, y, width_height[0], width_height[1]
            )  # collision rect
            self.image_rect = kwargs.get(
                "image_rect",
                pygame.Rect(
                    width_height[0] / 2 - self.spritesheet.tw / 2,
        ...
        # - an image file -> fixed image -> store as Surface in self.image
            ...
            assert isinstance(
                image, str
            ), "ERROR: in Sprite's ctor: kwargs[`image_file`] must be of type str!"
            self.spritesheet = None
            source = pygame.image.load(image)
            if "image_section" in kwargs:
                sec = kwargs["image_section"]
                assert (
                    isinstance(sec, tuple) and len(sec) == 4
            ...
            width_height = kwargs.get(
                "width_height", (self.image_rect.width, self.image_rect.height)
            )
            ...
            self.image_rect = kwargs.get(
                "image_rect",
                pygame.Rect(
                    width_height[0] / 2 - self.image_rect.width / 2,
        ...
        # - empty image plus a collision rect of some size
            ...
            assert (
                isinstance(width_height, tuple) and len(width_height) == 2
        ...
        self.type = Sprite.get_type(
            "default"
        )  # specifies the type of the Sprite (can be used e.g. for collision detection)
        self.handles_own_collisions = (
            False  # set to True if this object takes care of its own collision handling
        )
        self.collision_mask = Sprite.get_type(
            "default"
        )  # set the bits here that we would like to collide with (all other types will be ignored)

        self.stage = None  # the current Stage this Sprite is in
        self.sprite_groups = []  # the current Groups that this Sprite belongs to
        self.do_render = kwargs.get(
            "do_render", self.do_render
        )  # we may overwrite this
        self.render_order = kwargs.get(
            "render_order", 50
        )  # the higher this number the later this Sprite will be rendered in the Stage's render function
        self.flip = {
            "x": False,
            "y": False,
        }  # 'x': flip in x direction, 'y': flip in y direction, False: don't flip
        ...
        self.on_event(
            "added_to_stage", self, "added_to_stage", register=True
        )  # allow any Stage to trigger this event using this Sprite
    ...
    def render(self, display):
        ...
        if self.image:
            ...
            display.surface.blit(
                self.image,
                (
                    self.rect.x + self.image_rect.x - display.offsets[0],
                    self.rect.y + self.image_rect.y - display.offsets[1],
                ),
            )
        if DEBUG_FLAGS & DEBUG_RENDER_SPRITES_RECTS:
            pygame.draw.rect(
                ...
                pygame.Rect(
                    (
                        self.rect.x - display.offsets[0],
                        self.rect.y - display.offsets[1],
                    ),
