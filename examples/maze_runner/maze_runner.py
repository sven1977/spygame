...
from spygame.components.brains import SimpleHumanBrain
from spygame.game import Game
from spygame.physics.top_down_physics import TopDownPhysics
from spygame.screens import Level
from spygame.sprites import Sprite, SpriteSheet


class MyAgent(Sprite):
    def __init__(self, x, y):
        super().__init__(x, y, sprite_sheet=SpriteSheet("data/erik.tsx"), tile=0)
        ...
        self.cmp_brain = self.add_component(
            SimpleHumanBrain("brain", ["up", "down", "left", "right"])
        )
        # add a physics component to physics handling (here we use: simple 2D top-down view and controls)
        self.cmp_physics = self.add_component(TopDownPhysics("physics"))
    ...
    game = Game(
        screens_and_levels=[
            # the only level
            {
                "class": Level,
                "name": "MAZE",
                "id": 1,  # <- this will read the data/maze.tmx file for the level's layout and setup data
            },
